...
/**
 * GET /api/courses — list, scoped to the caller (NC-API-001/AC-4,
 * NC-API-007). `personId` filters to one tracked Person's courses ("me" for
 * the account owner's own, unfiltered when omitted — People feature).
 */
...
/**
 * POST /api/courses — create (SPEC-API-004 shared_schemas CoursePayload). A
 * non-null person_id must reference a Person the caller owns (People
 * feature) — the guard_course_person_ownership DB trigger backstops this.
 */
