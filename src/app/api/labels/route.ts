...
import { labelPayloadSchema } from "@/lib/api/schemas";
import { successResponse, validationErrorResponse, serverErrorResponse } from "@/lib/api/response";

/** GET /api/labels — list, scoped to the caller (NC-API-001/AC-4, NC-API-007). */
export async function GET(request: NextRequest) {
  ...
  if (error) return serverErrorResponse("labels list failed", error);
...
export async function POST(request: NextRequest) {
  ...
  const parsed = labelPayloadSchema.safeParse(await request.json().catch(() => null));
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .single();
  if (error) return serverErrorResponse("label create failed", error);
