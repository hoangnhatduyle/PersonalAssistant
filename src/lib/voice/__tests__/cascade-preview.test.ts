...
describe("previewCourseDeleteCascade / formatCascadeDisclosure", () => {
  ...
  it("counts live deadlines and their live reminders", async () => {
    ...
    expect(preview).toEqual({ deadlinesAffected: 2, remindersLive: 2, notesAffected: 0, boardCardsAffected: 0 });
  });
  ...
  it("counts notes linked to a course with no live deadlines (architect-review finding: don't claim nothing is affected)", async () => {
    ...
    const preview = await previewCourseDeleteCascade(user.client, userId, courseId);
    expect(preview).toEqual({ deadlinesAffected: 0, remindersLive: 0, notesAffected: 1, boardCardsAffected: 0 });
  ...
  it("reports a fully-empty course accurately", async () => {
    ...
    const preview = await previewCourseDeleteCascade(user.client, userId, courseId);
    expect(preview).toEqual({ deadlinesAffected: 0, remindersLive: 0, notesAffected: 0, boardCardsAffected: 0 });
    expect(formatCascadeDisclosure(preview)).toMatch(/no deadlines, linked notes, or board cards/i);
  });

  it("formats a combined disclosure for deadlines, reminders, notes, and board cards together", () => {
    const message = formatCascadeDisclosure({ deadlinesAffected: 2, remindersLive: 1, notesAffected: 3, boardCardsAffected: 4 });
    ...
    expect(message).toMatch(/delete its board list \(4 cards\)/);
  });
});
